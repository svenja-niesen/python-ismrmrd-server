...
#                     # old_grid = np.asarray(old_grid)
                    
#                     # data_abs = np.abs(data)
#                     # data_angle = np.unwrap(np.angle(data))
#                     # sensmaps_abs = intp_axis(newgrid=new_grid, oldgrid=old_grid, data=data_abs, axis=0) # Ergebnis als numpy array
#                     # sensmaps_angle = intp_axis(newgrid=new_grid, oldgrid=old_grid, data=data_angle, axis=0)
#                     # sensmaps = sensmaps_abs * np.exp(1j * sensmaps_angle)
                    
#                     # data_real = np.real(data)
#                     # data_imag = np.imag(data)
#                     # sensmaps_real = intp_axis(newgrid=new_grid, oldgrid=old_grid, data=data_real, axis=0) # Ergebnis als numpy array
#                     # sensmaps_imag = intp_axis(newgrid=new_grid, oldgrid=old_grid, data=data_imag, axis=0)
#                     # sensmaps = sensmaps_real + 1j * sensmaps_imag
...
#                 ti = np.zeros([metadata.encoding[0].encodedSpace.matrixSize.z, metadata.encoding[0].encodingLimits.kspace_encoding_step_1.maximum + 1])
#                 for i,acq in enumerate(group):
#                     ti[acq.idx.kspace_encode_step_2, acq.idx.kspace_encode_step_1] = i
#                 ti = tr * (dummies + ti) # [s]
...
#         np.save(debugFolder + "/" + "trjnew.npy", traj)
...
def process_raw(group, config, metadata, dmtx=None, sensmaps=None, gpu=False, prot_arrays=None):
    ...
    if gpu:
        ...
        nufft_config = 'nufft -i -l 0.005 -t -d %d:%d:%d'%(nx, nx, nz)
        ecalib_config = 'ecalib -m 1 -I -r 30 -k 4'
        pics_config = 'pics -S -e -l1 -r 0.001 -i 50 -t'
